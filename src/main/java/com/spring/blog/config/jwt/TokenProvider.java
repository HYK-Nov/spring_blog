...
public class TokenProvider {
    ...
    private String makeToken(Date expiry, User user){
        ...
        return Jwts.builder()   // 빌더 패턴으로 토큰 생성
                ...
                .setIssuer(jwtProperties.getIssuer())   // application.yml에 기입된 내용 자동 전달
                .setIssuedAt(now)   // 발급 시각
                .setExpiration(expiry)  // 만기 시각
                .setSubject(user.getLoginId())  // 기본적으로는 아이디
                .claim("id", user.getId())  // 클레임에는 PK 제공
                .signWith(SignatureAlgorithm.HS256, jwtProperties.getSecretKey())   // 알고리즘과 비밀키 같이 입력
                .compact(); // 생성메서드는 빌더패턴이지만 .build()가 아닌 .compact();
    }
    
//    검증 로직은 유효한지, 혹은 유효하지 않은지만 리턴하면 되므로 boolean을 리턴
//    발급된 토큰을 입력받았을 때, 유효하면 true, 아니면 false를 리턴
    public boolean validToken(String token){
        try {
            Jwts.parser()   // 토큰을 먼저 파싱(읽어들임)하고
                    .setSigningKey(jwtProperties.getSecretKey())    // 복호화시에는 암호화 때 사용한 비밀키 입력
                    .parseClaimsJws(token); // 검증 타겟이 되는 토큰 입력
            return true;    // 아무일도 없으면 유효 토큰이므로 true 리턴
        }catch (Exception e){
            return false;   // 파싱 및 복호화 비교 중 문제가 발생하면 exception이 발생하므로, false 리턴
        }
    }

    public Authentication getAuthentication(String token){  // 토큰 입력 시 인증정보 읽어오기
        Claims claims = getClaims(token);   // getClaims()는 아래에 추가로 작성
...
//    클레임 정보(실제 개인정보)는 외부에서 직접적으로 호출할 수 없고, 오로지 토큰제공 클래스 내부에서만 호출되도록 처리
